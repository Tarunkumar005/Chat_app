...
import { Send, Trash2, Edit, X, Check, Clock, ArrowLeft } from 'lucide-react';
    ...
    return (
        <div className="flex flex-col h-full bg-gray-50">
            <header className="p-4 bg-white border-b border-gray-200 flex items-center gap-4 flex-shrink-0">
                {/* ✨ NEW: Back button for mobile screens */}
            ...
            <div className="flex-1 p-4 md:p-6 overflow-y-auto space-y-1">
                <AnimatePresence>
                    {messages.map((msg) => {
                        ...
                        return (
                        <motion.div key={msg._id} layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, transition: { duration: 0.2 } }} className={`group flex items-end gap-2 ${msg.sender._id === user.id ? 'justify-end' : 'justify-start'}`}>
                            ...
                            {msg.sender._id === user.id && isTemp && (
                                <div className="p-1 text-gray-400 animate-pulse"><Clock size={14} /></div>
                            )}
